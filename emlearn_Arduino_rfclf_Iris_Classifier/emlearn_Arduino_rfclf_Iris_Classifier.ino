...
void loop() {
  ...
  if (Serial.available()) {
    ...
    int32_t y_pred = RandomForestClassifier_predict(features, sizeof(features) / sizeof(features[0]));
    Serial.print(classes[y_pred]);
 
    // Compute the probabilities for each class
    Serial.print("\t Probability: ");
    int err = RandomForestClassifier_predict_proba(features, sizeof(features) / sizeof(features[0]), proba, 3);

    if (err == 0) {
      Serial.println(proba[y_pred] * 100.00, 2);
    }
  }
  delay(100);
}